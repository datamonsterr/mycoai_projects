...
- Code paths like `src/`, `docs/`, and `report/` refer to `research/` unless the backend/frontend repo is explicitly named.
...
- The `.opencode/rules/branch-naming.md` and `.opencode/rules/experiment-visualization.md` rules apply only to `research/` autoresearch work, not to backend, frontend, or general monorepo branches and charts.
- `backend/` and `frontend/` consume validated outputs from `research/src/experiments/retrieval/` and `research/src/experiments/kmeans_segmentation/`; keep producer and consumer docs in sync when those contracts change.
- Product repos MAY inspect experiment code to understand behavior, but they MUST reimplement that behavior locally and MUST NOT import directly from `research/`.
...
- Detailed project guidance remains in `CLAUDE.md` and `research/README.md`.
...
- Research notebook: `research/research/`
- Worker runtime: `research/.runtime/worktrees/` (gitignored)

### Worktree Lifecycle

Workers create `research/.runtime/worktrees/<experiment-id>` per run.
