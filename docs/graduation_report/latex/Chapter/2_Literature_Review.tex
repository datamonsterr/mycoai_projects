...
\textbf{Stage 1 --- Full-Image Preprocessing:} 1. \textbf{Resizing}: Images are standardized to ($256 \times 256$) pixels to ensure consistent input dimensions. 2. \textbf{Circle Detection}: The Hough Circle Transform detects the circular Petri dish boundary, producing a center (\(c_x, c_y\)) and radius (R). 3. \textbf{Background Masking}: Pixels outside the detected dish are masked to black, removing irrelevant background (labels, shadows, bench surface). 4. \textbf{Cropping}: The dish region is extracted, discarding the remainder.
...
For each candidate colony region (\(R_i\)) in HSV space: 1. Apply a second K=2 clustering on the V (value) channel pixels within (\(R_i\)) only. 2. Identify the bright cluster (higher mean V value) and the dim cluster. 3. Construct a mask retaining only pixels belonging to the bright cluster. 4. Shrink the mask via morphological erosion to remove borderline halo pixels.
...
\tightlist
\item
  \textbf{Canny Edge Detection}: Applied to the blurred grayscale image with thresholds (\(t_{\text{low}} = 30\)), (\(t_{\text{high}} = 80\)).
\item
...
\begin{itemize}
\item \textbf{Collections:} \texttt{myco\_fungi\_features\_full} (pretrained) and \texttt{myco\_fungi\_features\_full\_finetuned} (fine-tuned).
...
\caption{Staircase Chart}\label{fig:staircase}
\end{figure}
