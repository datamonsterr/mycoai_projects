\section{Fungal Species Retrieval Model}\label{chapter-2-fungal-species-retrieval-model}
...
Current state-of-the-art in fungal identification often uses Convolutional Neural Networks (CNNs) such as ResNet\textasciitilde{}\cite{he2016resnet}, EfficientNet\textasciitilde{}\cite{tan2019efficientnet}, or Vision Transformers (ViTs). However, these models often struggle with the ``long tail'' of rare species where few training examples exist. Image retrieval via instance-based learning is more robust for these cases, as it only requires a few reference examples per species.
...
Raw Petri dish images undergo a multi-stage preprocessing pipeline to isolate individual fungal colonies. Given an original microscopy image (I\_\{\text{raw}\} \in \mathbb{R}\^{}\{H \times W \times 3\}), the pipeline operates as follows:

\textbf{Stage 1 --- Full-Image Preprocessing:} 1. \textbf{Resizing}: Images are standardized to (256 \times 256) pixels to ensure consistent input dimensions. 2. \textbf{Circle Detection}: The Hough Circle Transform detects the circular Petri dish boundary, producing a center ((c\_x, c\_y)) and radius (R). 3. \textbf{Background Masking}: Pixels outside the detected dish are masked to black, removing irrelevant background (labels, shadows, bench surface). 4. \textbf{Cropping}: The dish region is extracted, discarding the remainder.

\textbf{Stage 2 --- Colony Segmentation:}

Three complementary approaches were developed for colony segmentation:

\paragraph{K-Means Clustering Segmentation}\label{k-means-clustering-segmentation}
...
A key innovation is the \textbf{Local K=2 Shrink} method, which addresses a specific failure mode: bright small colonies that produce light halos on certain media (notably MEA and YES). The flare phenomenon occurs when the agar near a colony appears bright, causing K-Means to group the halo with the colony foreground rather than the background. The Local K=2 method operates as follows:

For each candidate colony region (R\_i) in HSV space: 1. Apply a second K=2 clustering on the V (value) channel pixels within (R\_i) only. 2. Identify the bright cluster (higher mean V value) and the dim cluster. 3. Construct a mask retaining only pixels belonging to the bright cluster. 4. Shrink the mask via morphological erosion to remove borderline halo pixels.
...
\paragraph{Contour-Based Segmentation}\label{contour-based-segmentation}
...
\tightlist
\item
  \textbf{Canny Edge Detection}: Applied to the blurred grayscale image with thresholds (t\_\{\text{low}\} = 30), (t\_\{\text{high}\} = 80).
\item
  \textbf{Morphological Closing}: Dilation ((5 \times 5) kernel, 3 iterations) followed by erosion ((3 \times 3) kernel, 2 iterations) to seal gaps in colony edges.
\item
  \textbf{Circularity Filter}: Contours are scored by (\text{area} \times \text{circularity}), where circularity is defined as (4\pi A / P\^{}2) ((A) = contour area, (P) = perimeter). A perfect circle scores 1.0. Contours are ranked and the top-3 are selected per dish, subject to area constraints ((400 \leq A \leq 23,700) px(\^{}2)).
\item
  \textbf{Bounding Box Extraction}: For each selected contour, the axis-aligned bounding rectangle is computed; boxes with area below the minimum threshold are filtered.
\end{enumerate}

This approach is more robust to uneven illumination but less reliable when colonies touch or overlap.

\paragraph{YOLO-Based Segmentation}\label{yolo-based-segmentation}
...
\tightlist
\item
  \textbf{Dataset}: \texttt{Dataset/manual\_labeled\_data\_roboflow\_species/}, annotated with colony bounding boxes and species labels derived from \texttt{Dataset/strain\_to\_specy.csv}.
\item
  \textbf{Split}: Deterministic random train/test split with no separate validation partition.
\item
  \textbf{Model}: YOLOv8n-seg (nano variant, (\sim)3.2M parameters) trained for 50 epochs with image size 640, batch size 16.
\item
  \textbf{Output}: Instance masks and bounding boxes for each detected colony, along with a confidence score (p \in [0,1]).
\end{itemize}
...
\begin{figure}
\centering
\includegraphics[width=\textwidth,keepaspectratio]{figures/pipeline_montage.jpg}
\caption{Preprocessing Pipeline}
\end{figure}

\subsubsection{Feature Extraction and Embedding}\label{feature-extraction-and-embedding}

Each segmented colony image is passed through a feature extractor (f\_\theta: \mathbb{R}\^{}\{256 \times 256 \times 3\} \to \mathbb{R}\^{}d) that maps the image to a high-dimensional embedding vector. All extracted features are L2-normalized to unit length for cosine similarity comparison:

{[} \mathbf{v} = \frac{f_\theta(\mathbf{x})}{\|f_\theta(\mathbf{x})\|_2} {]}

\textbf{Hand-Crafted Extractors:}
...
\textbf{Deep Learning Extractors:}
...
\begin{figure}
\centering
\includegraphics[width=\textwidth,keepaspectratio]{figures/training_curves.png}
...
\textbf{Alternative: Triplet Loss Training:}
...
{[} \mathcal{L}\_\{\text{triplet}\} = \max\left(0,; \textbar f(\mathbf{x}\_a) - f(\mathbf{x}\_p)\textbar\_2 - \textbar f(\mathbf{x}\_a) - f(\mathbf{x}\_n)\textbar\_2 + \alpha\right) {]}
...
\subsubsection{Vector Database Retrieval}\label{vector-database-retrieval}
...
Let the strain produce (m) query segments, and let the (i)-th segment retrieve (k) neighbors with species labels (c\_\{i,1\}, \ldots, c\_\{i,k\}) and similarity scores (s\_\{i,1\}, \ldots, s\_\{i,k\}).

\textbf{Weighted Aggregation (default):} For each candidate species (c),

{[} S\_c\^{}\{\text{weighted}\} = \frac{\sum_{i=1}^{m} \sum_{j=1}^{k} s_{i,j} \cdot \mathbf{1}[c_{i,j} = c]}{\sum_{i=1}^{m} \sum_{j=1}^{k} \mathbf{1}[c_{i,j} = c]} {]}
...
{[} S\_c\^{}\{\text{uni}\} = \frac{1}{m \cdot k} \sum\emph{\{i=1\}\^{}\{m\} \sum}\{j=1\}\^{}\{k\} \mathbf{1}{[}c\_\{i,j\} = c{]} {]}

The weighted strategy is more robust to outliers and is used as the default.

The final predicted species is:

{[} \hat{c} = \underset{c}{\arg\max}; S\_c {]}

with confidence (\text{conf}(\hat{c}) = S\_\{\hat{c}\}).

\subsubsection{Environment Selection Strategies}\label{environment-selection-strategies}
...
Multiple feature extractors can be combined via weighted score fusion. Given extractors (f\_1, f\_2, \ldots, f\_n) with per-extractor scores (S\_c\^{}\{(t)\}) and weights (w\_t):

{[} S\_c\^{}\{\text{ensemble}\} = \sum\_\{t=1\}\^{}\{n\} w\_t \cdot S\_c\^{}\{(t)\} {]}
...
\subsubsection{Dataset and Split Strategy}\label{dataset-and-split-strategy}
...
\subsubsection{Segmentation Experiments}\label{segmentation-experiments}
...
\subsubsection{Retrieval Experiments: Staircase Chart}\label{retrieval-experiments-staircase-chart}
...
The staircase chart (Figure\textasciitilde{}\ref{fig:staircase}) plots experiment index on the x-axis and F1 score on the y-axis. Gray dots represent experiments below the running best; green dots set a new best and step the staircase upward. Each green dot is labeled with the formula-algorithm pair that achieved the improvement.

\begin{figure}
\centering
\includegraphics[width=\textwidth,keepaspectratio]{figures/staircase_retrieval.png}
\caption{Staircase Chart}
\end{figure}

\subsubsection{Overall Performance Comparison}\label{overall-performance-comparison}
...
\subsubsection{Cross-Validation Results}\label{cross-validation-results}
...
\begin{figure}
\centering
\includegraphics[width=\textwidth,keepaspectratio]{figures/accuracy_vs_k.png}
...
\begin{figure}
\centering
\includegraphics[width=\textwidth,keepaspectratio]{figures/fold_variance.png}
\caption{Fold Variance Box Plot}
\end{figure}
...
\begin{figure}
\centering
\includegraphics[width=\textwidth,keepaspectratio]{figures/heatmap_env_strategy_k.png}
\caption{Heatmap: env (\times) strategy vs K}
\end{figure}

\begin{figure}
\centering
\includegraphics[width=\textwidth,keepaspectratio]{figures/env_comparison.png}
\caption{E1 vs E2 Bar Chart}
\end{figure}

\subsubsection{Feature Space Analysis (t-SNE)}\label{feature-space-analysis-t-sne}
...
\subsubsection{Confusion Matrix and Per-Species Analysis}\label{confusion-matrix-and-per-species-analysis}
...
\begin{figure}
\centering
\includegraphics[width=\textwidth,keepaspectratio]{figures/confusion_matrix_efficientnetb1.png}
\caption{Confusion Matrix}
\end{figure}

\subsubsection{Prediction Examples}\label{prediction-examples}
