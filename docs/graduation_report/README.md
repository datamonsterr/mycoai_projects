...
uv --directory research run python graduation_report/code/chart_experiment_results.py
...
Charts are saved to both `graduation_report/figures/` (LaTeX) and `graduation_report/report/figures/` (HTML report).
...
`drawio-ai-kit` is vendored at `tools/drawio-ai-kit/` so thesis diagram source and tooling stay pinned in repo state. Thesis source now lives in the `graduation_report/` git submodule; keep all thesis code, chapter edits, and figure source updates there instead of under `docs/graduation_report/`. Local use still requires `npm install` in the vendored path and a locally installed draw.io desktop CLI for PNG export.
...
|--------|----------------|------|--------------------------|
| `ch03_architecture.png` | `graduation_report/Chapter/Web_Application.tex` | system architecture | `graduation_report/figures/src/ch03_architecture.drawio` |
| `ch03_erd.png` | `graduation_report/Chapter/Web_Application.tex` | PostgreSQL ERD | `graduation_report/figures/src/ch03_erd.dbml` (DBML) |
| `ch02_research_pipeline.png` | `graduation_report/Chapter/Retrieval_Model_Research.tex` | methodology pipeline | `graduation_report/figures/src/ch02_research_pipeline.drawio` |
| `threshold_pipeline_diagram.png` | `graduation_report/Chapter/Retrieval_Model_Research.tex` | threshold algorithm flow | `graduation_report/figures/src/threshold_pipeline_diagram.drawio` |

Migration exclusions are explicit:

- Keep `ch03_usecase_diagram.png` on current source path and workflow.
- Keep sequence diagrams Mermaid-sourced with neutral grey theme: `ch03_auth_sequence.png`, `ch03_srs_retrieve_sequence.png`, `ch03_srs_index_sequence.png`, `ch03_srs_feedback_sequence.png`. Sources live as `.mmd` files in `graduation_report/figures/src/`, rendered via `graduation_report/code/render_mermaid_ink.py` (mermaid.ink API).
...
bash graduation_report/code/render_erd.sh
```

Requires `dbml-renderer` and `rsvg-convert` (librsvg). Source is `graduation_report/figures/src/ch03_erd.dbml`.

#### Sequence Diagram Export (Mermaid → PNG, neutral theme)

```bash
python graduation_report/code/render_mermaid_ink.py \
  graduation_report/figures/src/<name>.mmd \
  graduation_report/figures/<name>.png
```
...
1. Keep editable `.drawio` sources under `graduation_report/figures/src/`.
2. Export PNG outputs into `graduation_report/figures/` with filenames matching the LaTeX includes.
...
```bash
"${DRAWIO_CLI:-drawio}" --export --format png --output graduation_report/figures/<figure-name>.png graduation_report/figures/src/<figure-name>.drawio
```

Headless Chromium export fallback (no draw.io desktop needed):

```bash
NODE_PATH=/tmp/puppeteer/node_modules node graduation_report/code/export_drawio_headless.cjs
```
...
npm install --prefix tools/drawio-ai-kit
npm test --prefix tools/drawio-ai-kit
python graduation_report/code/export_drawio_diagrams.py
cd graduation_report
...
- `graduation_report/content/methodology_pipeline.md` currently holds Mermaid methodology source semantics that upcoming draw.io work should preserve.


### New Figure Integration

1. Generate the PNG in `graduation_report/figures/`
...
2. Run chart generation:
   ...
   uv --directory research run python graduation_report/code/chart_experiment_results.py
   ```
3. Verify new charts in `graduation_report/figures/`
...
6. Recompile and verify zero warnings:
   ```bash
   cd graduation_report && pdflatex main.tex && bibtex main && pdflatex main.tex && pdflatex main.tex
   ```

### Adding a New Chart Type

1. Write generation code in `graduation_report/code/` — one file per chart family
