...
describe('startQuery', () => {
  it('POSTs to /retrieval/query and returns RetrievalJobResponse', async () => {
    ...
    const result = await startQuery({ image_id: 'img-1', k: 5, aggregation: 'weighted', environment_strategy: 'mean' })
  ...
  it('throws ApiError on non-ok response', async () => {
    ...
    await expect(startQuery({ image_id: 'bad-id', k: 5, aggregation: 'weighted', environment_strategy: 'mean' }))
...
describe('querySync', () => {
  it('POSTs to /retrieval/query-sync and returns results', async () => {
    ...
    const result = await querySync({ image_id: 'img-2', k: 3, aggregation: 'uni', environment_strategy: 'max' })
