"""CLI wrapper for threshold experiment.

Usage:
    uv run python -m src.experiments.threshold.cli \
        --run-id <run_id> \
        --output-root <path> \
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Threshold experiment CLI")
    ...
    try:
        from src.experiments.threshold.run import ExperimentParams, run
