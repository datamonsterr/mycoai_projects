...
from src.config import CURATED_METADATA_PATH, RESULTS_DIR
...
def _filter_labels(labels: List[str]) -> List[str]:
    ...
    filtered = sorted(l for l in all_labels if l != "UNKNOWN")
    if "UNKNOWN" in all_labels:
        filtered.append("UNKNOWN")
    return filtered


def draw_standard(cm_df: pd.DataFrame, accuracy: float, known_count: int, total: int) -> None:
    ...
    sns.heatmap(
        cm_df, annot=True, fmt="d", xticklabels=labels, yticklabels=labels,
...
def draw_with_unknown_highlighted(cm_df: pd.DataFrame, accuracy: float, known_count: int, total: int) -> None:
    labels = list(cm_df.columns)
    n = len(labels)
    ...
    sns.heatmap(
        cm_df, annot=annot_matrix, fmt="", xticklabels=labels, yticklabels=labels,
        cmap="YlOrRd", ax=ax, linewidths=0.5, linecolor="white",
        annot_kws={"fontsize": 6}, cbar_kws={"shrink": 0.8},
    )
    ...
    if "UNKNOWN" in labels:
        ...
        for j in range(n):
            rect = plt.Rectangle((j, uidx), 1, 1, fill=False, edgecolor="red", linewidth=2.5)
            ax.add_patch(rect)
        for i in range(n):
            rect = plt.Rectangle((uidx, i), 1, 1, fill=False, edgecolor="red", linewidth=2.5)
            ax.add_patch(rect)
...
def draw_simplified(y_true: List[str], y_pred: List[str], accuracy: float, known_count: int, total: int) -> None:
    ...
    sns.heatmap(
        cm_df_simple, annot=annot_s, fmt="", xticklabels=all_simple, yticklabels=all_simple,
    ...
    if "UNKNOWN" in all_simple:
        ...
        for j in range(n):
            rect = plt.Rectangle((j, uidx), 1, 1, fill=False, edgecolor="red", linewidth=3)
            ax.add_patch(rect)
...
def print_summary(y_true: List[str], y_pred: List[str], df: pd.DataFrame) -> None:
    ...
    correct_known = sum(1 for yt, yp in zip(y_true, y_pred) if yt == yp and yt != "UNKNOWN")
