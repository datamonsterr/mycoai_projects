...
def main() -> None:
    ...
    df["top3_gmean"] = stats.gmean(df[["s0", "s1", "s2"]].clip(lower=1e-10), axis=1)
    ...
    fig.suptitle(
        "Threshold Experiment — Score Distributions", fontsize=16, fontweight="bold"
    )
    ...
    known_df["correct"] = known_df.apply(
        lambda r: (
            _strip_penicillium(str(r["species_label"]))
            == _strip_penicillium(str(r["predicted_species"]))
        ),
        axis=1,
    )
    ...
    ax4.scatter(
        wrong_pts["s0"],
        wrong_pts["top3_gmean"],
        c="tomato",
        alpha=0.5,
        s=15,
        label=f"Wrong ({len(wrong_pts)})",
    )
    ax4.scatter(
        correct_pts["s0"],
        correct_pts["top3_gmean"],
        c="forestgreen",
        alpha=0.7,
        s=20,
        label=f"Correct ({len(correct_pts)})",
    )
    ...
    print("\n=== Score Stats ===")
    print(
        f"s0 known   mean: {know_mean:.4f}, median: {known['s0'].median():.4f}, std: {known['s0'].std():.4f}"
    )
    print(
        f"s0 unknown mean: {unk_mean:.4f}, median: {unknown['s0'].median():.4f}, std: {unknown['s0'].std():.4f}"
    )
    print(f"top3 gmean known   mean: {k_gmean:.4f}, median: {known_gmean.median():.4f}")
    print(
        f"top3 gmean unknown mean: {u_gmean:.4f}, median: {unknown_gmean.median():.4f}"
    )
