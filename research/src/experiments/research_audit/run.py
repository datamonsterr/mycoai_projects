...
def _summarize_reports() -> dict[str, Any]:
    ...
    report_dirs = (
        sorted([p.name for p in report_root.iterdir() if p.is_dir()])
        if report_root.exists()
        else []
    )
    graduation_files = (
        sorted(
            [p.name for p in (docs_root / "graduation_report" / "content").iterdir()]
        )
        if (docs_root / "graduation_report" / "content").exists()
        else []
    )
    return {
...
def _build_inventory() -> dict[str, Any]:
    effective_mapping_rows, effective_species_count = _mapping_summary(
        STRAIN_SPECIES_MAPPING_PATH
    )
    monorepo_mapping_rows, monorepo_species_count = _mapping_summary(
        MONOREPO_DATASET_ROOT / "strain_to_specy.csv"
    )

    effective_weight_files = (
        sorted([p.name for p in WEIGHTS_DIR.iterdir()]) if WEIGHTS_DIR.exists() else []
    )
    monorepo_weight_files = (
        sorted([p.name for p in MONOREPO_WEIGHTS_DIR.iterdir()])
        if MONOREPO_WEIGHTS_DIR.exists()
        else []
    )
    ...
    return {
        "effective": {
            ...
            "prepared_segments_count": _load_json_count(
                PREPARED_SEGMENTS_METADATA_PATH
            ),
        ...
        "monorepo": {
            ...
            "prepared_items_count": _load_json_count(
                MONOREPO_DATASET_ROOT / "prepared_items_metadata.json"
            ),
            "prepared_segments_count": _load_json_count(
                MONOREPO_DATASET_ROOT / "prepared_segments_metadata.json"
            ),
...
def _build_risk_flags(inventory: dict[str, Any], qdrant_state: dict[str, Any]) -> str:
    ...
    risks.append(
        "- Historical metrics are untrusted until rerun under fresh-query leakage-safe protocol."
    )
    risks.append(
        "- Fine-tune training currently depends on a global Test column and needs fold-safe manifests."
    )
    risks.append(
        "- Retrieval benchmark code must be forced through fresh image query path with held-out strain exclusion."
    )
    if effective["dataset_root"] != monorepo["dataset_root"]:
        risks.append(
            "- Config drift: research runtime resolves local research/ Dataset, results, and weights instead of monorepo shared paths."
        )
    if not monorepo.get("weight_files"):
        risks.append(
            "- No weights found in monorepo weights/; finetuned benchmark cannot be reproduced without retraining or restoring weights."
        )
...
def main() -> None:
    ...
    (AUDIT_DIR / "qdrant_collections.json").write_text(
        json.dumps(qdrant_state, indent=2)
    )

    reports_summary = _summarize_reports()
    (AUDIT_DIR / "reports_summary.json").write_text(
