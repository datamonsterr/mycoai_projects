...
KNOWN_DISPLAY = [
    s.replace("penicillium-", "").replace("-", " ") for s in sorted(KNOWN_SPECIES)
]
...
def write():
    ...
    points, _ = client.scroll(
        collection_name="full_prepared_features", limit=3000, with_payload=True
    )
    for p in points:
        pl = p.payload or {}
    ...
    mapping_df = pd.read_csv(
        Path("/home/dat/dev/mycoai_projects/Dataset/strain_to_specy.csv")
    )
    canonical_lookup = {
        s.replace("penicillium ", "").replace(" ", "-").lower(): s
        for s in mapping_df["Species"].unique()
    }
    ...
    for species_slug in sorted(species_strains):
        if not is_known_species(species_slug):
            ...
            if strains:
                rows.append(
                    {"species": species_slug, "strain": strains[0], "is_known": 0}
                )
    ...
    ax.bar(
        ["Known", "Unknown"], [known_count, unknown_count], color=["#2ecc71", "#e74c3c"]
    )
    for i, v in enumerate([known_count, unknown_count]):
        ax.text(i, v + 0.5, str(v), ha="center", fontsize=12, fontweight="bold")
    ax.set_ylabel("Number of species")
    ax.set_title("Full-prepared dataset: known vs unknown species")
    for out in [
        GRAD_FIG / "threshold_known_unknown_distribution.png",
        LATEX_FIG / "threshold_known_unknown_distribution.png",
    ]:
    ...
    colors = [
        "#2ecc71" if sp in known_species_list else "#e74c3c" for sp in counts.index
    ]
    ...
    ax.set_ylabel("Count")
    ax.set_title("Species distribution in full_prepared dataset")
    for out in [
        GRAD_FIG / "threshold_full_species_distribution.png",
        LATEX_FIG / "threshold_full_species_distribution.png",
    ]:
        fig.savefig(out, dpi=200, bbox_inches="tight")
    plt.close(fig)
        ...
        json.dumps(
            {
                ...
                "csv": str(THRESHOLD_DIR / "known_unknown_strains.csv"),
            }
        )
    )
