...
def build_clean_strain_grid() -> Path:
    ...
    border_w = 4
    canvas = Image.new(
        "RGB", (left_w + len(media) * cell_w, top_h + len(rows) * cell_h), "white"
    )
    ...
    for row_idx, (species, strain, label, color_hex) in enumerate(rows):
        ...
        for col, medium in enumerate(media):
            ...
            image = (
                Image.open(source)
                .convert("RGB")
                .resize((cell_w - 2 * pad, cell_h - 2 * pad))
            )
            x0 = left_w + col * cell_w + pad
            y1 = y0 + pad
            canvas.paste(image, (x0, y1))
            draw.rectangle(
                [x0, y1, x0 + image.width, y1 + image.height],
                outline=color_hex,
                width=border_w,
            )
...
def build_segmentation_io() -> Path:
    ...
    for axis, segment, title in zip(
        axes[1:],
        segments,
        ["YOLO segment 1", "YOLO segment 2", "YOLO segment 3"],
        strict=False,
    ):
...
def build_kmeans_vs_yolo() -> Path:
    base = (
        ROOT / "Dataset" / "original_prepared" / "penicillium-polonicum" / "dto-148-d1"
    )
    samples = [
        base / "crea" / "ob",
        base / "cya30" / "ob",
    ...
    for col, sample in enumerate(samples):
        ...
        axes[0, col].set_title(
            f"KMeans - {sample.parent.name.upper()} {sample.name}", fontsize=11
        )
        axes[0, col].axis("off")
        axes[1, col].imshow(mpimg.imread(sample / "bbox_yolo.jpg"))
        axes[1, col].set_title(
            f"YOLO - {sample.parent.name.upper()} {sample.name}", fontsize=11
        )
        axes[1, col].axis("off")
    fig.tight_layout()
    fig.savefig(out, bbox_inches="tight")
...
def build_pipeline_steps_oneline() -> Path:
    ...
    step_images = [
        ...
        (
            "Location clusters",
            cv2.cvtColor(debug["location_clusters"], cv2.COLOR_BGR2RGB),
        ),
...
def build_knn_prediction_visuals() -> list[Path]:
    ...
    result_path = (
        ...
        / "results"
        / "retrieval_batch1"
        / "efficientnetb1_finetuned_5_freq_strength_E1_yolo"
    ...
    for strain, correct, predicted_specy, filename in targets:
        ...
        if match is None:
            raise ValueError(
                f"Missing prediction case for {strain} {predicted_specy} correct={correct}"
            )
